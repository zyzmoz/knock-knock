...
export const homeCtrl = () => {
  ...
  const showSingleListing = async (listing) => {
    ...
    descriptionP.innerHTML = `${listing.propertyDescription}`;
    ...
    m.setView([listing.propertyAddress.location.lat, listing.propertyAddress.location.lng], 13);
    L.marker([listing.propertyAddress.location.lat, listing.propertyAddress.location.lng]).addTo(m);
  ...
  const renderListings = (listingArr) => {
    listingMessage.innerHTML = `<b>Showing ${listingArr?.length || 0} apartment(s) and house(s)</b>`;
  ...
  propertyTypeFilter.addEventListener('change', (evt) => {
    ...
    const filteredList =
      evt.target.value === '' ? listings : listings.filter((l) => l.propertyType === evt.target.value);
      ...
      isNaN(price) || price == 0
        ...
        : listings.filter((l) => {
            switch (price) {
              ...
              case 2000:
                return l.propertyRentPrice >= 1000 && l.propertyRentPrice <= price;
              case 3000:
  ...
  propertyFeaturesFilter.addEventListener('change', (evt) => {
    ...
    const filteredList =
      evt.target.value === '' ? listings : listings.filter((l) => l.propertyFeatures?.isFurnished === furnished);
    renderListings(filteredList);
  });
  ...
  propertyAllowsPetsFilter.addEventListener('change', (evt) => {
    ...
    console.log(pets);
    const filteredList = evt.target.value === '' ? listings : listings.filter((l) => l.propertyAllowsPets === pets);
    renderListings(filteredList);
  });
};
